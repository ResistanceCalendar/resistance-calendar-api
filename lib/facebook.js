...
const geocodeFacebookLocation = function (facebookEvent, callback) {
  if (facebookEvent.place.location) {
    callback(null, facebookEvent.place.location);
  } else {
    console.info('No location info, geocoding for ' + facebookEvent.place.name);
    geocoder.geocode(facebookEvent.place.name, function (err, res) {
      if (err) {
        callback(err);
      }

      const location = res.results[0];
      const facebookEventLocation = {
        street: location.address_components[0].short_name + ' ' + location.address_components[1].short_name,
        city: location.address_components[2].short_name,
        state: location.address_components[4].short_name,
        zip: location.address_components[6].short_name,
        longitude: location.geometry.location.lng,
        latitude: location.geometry.location.lat
      };
      callback(null, facebookEventLocation);
    });
  }
};

module.exports.toOSDIEvent = function (facebookEvent, callback) {
  const identifier = 'facebook:' + facebookEvent.id;
  const originSystem = 'Facebook';

  var location;

  geocodeFacebookLocation(facebookEvent, function (err, facebookEventLocation) {
    if (err) {
      callback(err);
    }
